...
from direct_tree.direct_tree_utils import bfs_iterate_modules, duplicate_subtree, recursive_iterate_modules
...
def develop(genotype: DirectTreeGenotype) -> Body:
    # print(modules[1], genotype.genotype.grid_position(modules[1]))
    # print(genotype.body.genotype.grid_position(
    #     ([x for x in recursive_iterate_modules(genotype.body.genotype.core)][0][0])))
    genotype.genotype.finalize()
    modules = list(set([x[2] for x in recursive_iterate_modules(genotype.genotype.core)]))
    coordinates = [(x, list(genotype.genotype.grid_position(x))) for x in modules]
    return genotype.genotype, coordinates
