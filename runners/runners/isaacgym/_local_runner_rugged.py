...
class LocalRunner(Runner):
    class _Simulator:
        ...
        def _create_envs(self) -> List[GymEnv]:
            ...
            # heightfield[0:num_rows, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.0, max_height=0.2,
            #                                                     step=0.2, downsampled_scale=0.5).height_field_raw
        ...
        def run(self) -> List[RunnerState]:
            ...
            while (
                time2 := self._gym.get_sim_time(self._sim)
            ) < self._batch.simulation_time:
                # do control if it is time
                import time
                # time.sleep(1)

                if time2 >= last_control_time + control_step:
                    last_control_time = math.floor(time2 / control_step) * control_step
                ...
                if time2 >= last_sample_time + sample_step:
                    last_sample_time = int(time2 / sample_step) * sample_step
                    states.append(self._get_state(time2))
